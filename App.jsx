...
function AppPrincipal({ usuario, onLogout }) {
  ...
  // ── ALARME ────────────────────────────────────────────────────────────────
  // Toca uma rajada de 5 bipes urgentes num padrão que chama atenção.
  const dispararRajada = () => {
    try {
      ...
      const bipe = (freq, start, dur, vol = 0.55) => {
      ...
      // Padrão: BI-BI-BI-BI  BIIIIM  (4 curtos + 1 longo)
      bipe(1200, t + 0.00, 0.10)
      bipe(1200, t + 0.16, 0.10)
      bipe(1200, t + 0.32, 0.10)
      bipe(1200, t + 0.48, 0.10)
      bipe(880,  t + 0.72, 0.55, 0.5)   // tom grave sustentado
    } catch(e) {}
  }
  ...
  const iniciarAlarme = () => {
    ...
    dispararRajada()
    alarmTimerRef.current = setInterval(dispararRajada, 4000)
  }

  // Reage ao estado das reservas: inicia ou para o alarme automaticamente.
  // O alarme para SOMENTE quando não há mais reservas não vistas no dia.
  const temReservasNaoVistas = useMemo(() => {
    const hoje = toDateStr(new Date())
    return reservas
      .filter(r => r.data === hoje)
      .some(r => !reservasStatus[r.key]?.visualizado)
  }, [reservas, reservasStatus])

  useEffect(() => {
    if (temReservasNaoVistas) iniciarAlarme()
    else pararAlarme()
    return pararAlarme
